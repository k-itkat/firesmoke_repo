...
# new_data_day = (datetime.now() - timedelta(1)).strftime('%Y-%m-%d')
# link = 'https://cwfis.cfs.nrcan.gc.ca/maps/fm3?type=rpt&'
# dr = pd.date_range(start='1/1/2025', end=new_data_day)

# total_smoke = pd.Series(dtype=float)
# for i in dr:
#     dt = (i.date())
#     dt = str(dt)
#     yr = dt[0:4]
#     lt = link[0:48]+'year='+dt[0:4]+'&month='+dt[6]+'&day='+dt[8:10]
#     data = pd.read_html(lt)[0]
#     data = data.set_index('Agency')['Smoke (km2)']
#     print(lt)
#     total_smoke = total_smoke.add(data,fill_value=0)
# print(total_smoke)
# total_smoke.plot.bar()to