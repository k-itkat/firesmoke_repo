...
appended = []
for year in (years):
    linkyear = link[0:48]+'year='+str(year)+'&month='+today[6:7]+'&day='+today[8:10]
    print(linkyear)
    data = pd.read_html(linkyear)[0]
    data = data.set_index('Agency')
    #data = data.rename(columns={'Smoke (km2)': year})
    data = data['Smoke (km2)']
    data.rename(year, inplace=True)
    #data = data.rename(columns={'Smoke (km2)': year})
    appended.append(data)       
appended = pd.concat(appended,axis=1)
bc = appended.loc['AB']
print(appended)
appended.plot.bar()
    #print(data[year])
